...
export default function DashboardWidget({ widget, onRetry }) {
  ...
  return (
    <article className="flex min-h-[232px] flex-col rounded-2xl border border-[#E2E8F0] bg-white p-5 shadow-[0_12px_35px_rgba(15,23,42,0.05)] transition hover:-translate-y-0.5 hover:shadow-[0_16px_38px_rgba(15,23,42,0.08)] dark:border-[#263346] dark:bg-[#151D2A] dark:shadow-[0_12px_35px_rgba(0,0,0,0.18)]">
      ...
      <div className="mt-5">
        <p className="text-sm font-medium text-[#64748B] dark:text-[#94A3B8]">{widget.title}</p>
        <p className={`mt-1 text-2xl font-bold tracking-tight ${isUnavailable ? "text-[#94A3B8]" : "text-[#1E293B] dark:text-[#F8FAFC]"}`}>{widget.value}</p>
      </div>
      <div className="mt-auto pt-4"><WidgetState widget={widget} onRetry={onRetry} /><WidgetVisual widget={widget} /></div>
    </article>
  )
}