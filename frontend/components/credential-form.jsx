...
import { login, registerCompany } from '@/src/api'
    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center overflow-y-auto bg-slate-950/60 px-4 py-8 backdrop-blur-sm" role="dialog" aria-modal="true" aria-labelledby="credential-title">
      ...
      <div className="relative z-10 w-full max-w-md rounded-3xl border border-[#E2E8F0] bg-white p-7 shadow-2xl shadow-slate-950/20 dark:border-[#263346] dark:bg-[#151D2A] dark:shadow-black/50 sm:p-9">
        ...
        <form className="flex flex-col gap-5" onSubmit={handleSubmit}>
          {isRegister && (
            <>
            <label className="flex flex-col gap-2 text-sm font-medium text-[#1E293B] dark:text-[#F8FAFC]" htmlFor="credential-company">
              Empresa
              <span className="relative">
                ...
                <input id="credential-company" name="companyName" type="text" autoComplete="organization" placeholder="Tu empresa" required className="h-12 w-full rounded-xl border border-[#E2E8F0] bg-[#F8F9FA] px-11 text-sm text-[#1E293B] outline-none transition placeholder:text-[#94A3B8] hover:border-brand-blue/70 focus:border-brand-blue focus:ring-4 focus:ring-brand-blue/10 dark:border-[#263346] dark:bg-[#0B0F17] dark:text-[#F8FAFC] dark:hover:border-brand/70 dark:focus:border-brand dark:focus:ring-brand/10" />
              </span>
            </label>
            <div className="grid gap-5 sm:grid-cols-2">
              <label className="flex flex-col gap-2 text-sm font-medium text-[#1E293B] dark:text-[#F8FAFC]" htmlFor="credential-first-name">
                Nombre
                <input id="credential-first-name" name="firstName" type="text" autoComplete="given-name" required className="h-12 w-full rounded-xl border border-[#E2E8F0] bg-[#F8F9FA] px-4 text-sm text-[#1E293B] outline-none transition focus:border-brand-blue focus:ring-4 focus:ring-brand-blue/10 dark:border-[#263346] dark:bg-[#0B0F17] dark:text-[#F8FAFC] dark:focus:border-brand" />
              </label>
              <label className="flex flex-col gap-2 text-sm font-medium text-[#1E293B] dark:text-[#F8FAFC]" htmlFor="credential-last-name">
                Apellido
                <input id="credential-last-name" name="lastName" type="text" autoComplete="family-name" required className="h-12 w-full rounded-xl border border-[#E2E8F0] bg-[#F8F9FA] px-4 text-sm text-[#1E293B] outline-none transition focus:border-brand-blue focus:ring-4 focus:ring-brand-blue/10 dark:border-[#263346] dark:bg-[#0B0F17] dark:text-[#F8FAFC] dark:focus:border-brand" />
              </label>
            </div>
            </>
          )}
          ...
          {errorMessage && <p role="alert" className="rounded-lg bg-rose-50 px-3 py-2 text-sm text-rose-700 dark:bg-rose-500/10 dark:text-rose-300">{errorMessage}</p>}
          <button type="submit" disabled={isSubmitting} className="mt-1 inline-flex h-12 items-center justify-center gap-2 rounded-xl bg-brand-blue px-4 text-sm font-semibold text-white shadow-lg shadow-blue-600/25 transition hover:bg-blue-700 focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-brand-blue/30 active:scale-[0.99] disabled:cursor-wait disabled:opacity-60 dark:bg-brand dark:text-[#0B0F17] dark:shadow-[0_0_20px_rgba(249,115,22,0.3)] dark:hover:bg-brand-hover dark:focus-visible:ring-brand/30">
